...
export default function InviteAccept({ token, onDone }) {
  ...
  const card = (children) => (
    <div style={{ minHeight: '100vh', background: 'var(--surface)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ width: '100%', maxWidth: 420, background: '#fff', borderRadius: 16, border: '1px solid var(--border)', boxShadow: '0 4px 24px rgba(0,0,0,0.07)', overflow: 'hidden' }}>
        <div style={{ padding: '24px 28px 0', display: 'flex', alignItems: 'center', gap: 10, marginBottom: 24 }}>
          <div style={{ width: 40, height: 40, borderRadius: 10, background: 'var(--accent-bg)', border: '1px solid var(--accent-border)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <LogoMark />
          </div>
          <div>
            <div style={{ fontSize: 15, fontWeight: 800 }}>
              <span style={{ color: 'var(--text)' }}>Mail </span>
              <span style={{ color: 'var(--sky)' }}>IQ</span>
            </div>
            <div style={{ fontSize: 9, letterSpacing: '0.18em', textTransform: 'uppercase', color: 'var(--dim)', fontWeight: 600 }}>
  ...
  if (expired) return card(
    <>
      <h2 style={{ fontSize: 16, fontWeight: 700, color: 'var(--text)', marginBottom: 8 }}>Convite expirado</h2>
  ...
  if (error && !info) return card(
    <>
      <h2 style={{ fontSize: 16, fontWeight: 700, color: 'var(--text)', marginBottom: 8 }}>Link inválido</h2>
  ...
  if (done) return card(
    <>
      <div style={{ textAlign: 'center', marginBottom: 16 }}>
        <div style={{ width: 48, height: 48, borderRadius: '50%', background: 'var(--ok-bg)', border: '1px solid var(--ok-border)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 12px' }}>
          <span style={{ fontSize: 22 }}>✓</span>
        </div>
        <h2 style={{ fontSize: 16, fontWeight: 700, color: 'var(--text)', marginBottom: 6 }}>Conta ativada!</h2>
        <p style={{ fontSize: 13, color: 'var(--muted)', marginBottom: 20 }}>
          Sua conta foi criada com sucesso. Faça login para continuar.
        </p>
        <button
          onClick={onDone}
          style={{ padding: '10px 24px', borderRadius: 8, fontSize: 13, fontWeight: 700, border: 'none', background: 'var(--sky)', color: '#fff', cursor: 'pointer' }}>
          Ir para o login
        </button>
      </div>
    </>
  ...
  return card(
    <>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        {error && (
          <div style={{ padding: '8px 12px', borderRadius: 8, background: 'var(--danger-bg)', border: '1px solid var(--danger-border)', color: 'var(--danger)', fontSize: 12 }}>
            {error}
          </div>
        )}
        ...
        <button
          ...
          style={{
            ...
            border: 'none', background: saving ? '#7DD3F0' : 'var(--sky)',
