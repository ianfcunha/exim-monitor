  ...
  return (
    ...
    >
      <span style={{
        ...
        background: active ? cfg.dot : '#CBD5E1',
        flexShrink: 0,
      }} />
      {cfg.label}
    </button>
...
export default function LogViewerDrawer({ onClose }) {
  ...
  const [filter, setFilter]       = useState('all')  // 'all' | tipo
  ...
  // Carrega na montagem e quando limit muda
  useEffect(() => {
    setLoading(true)
    load()
  }, [load])
      ...
      >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 10, color: '#94A3B8', fontFamily: 'monospace' }}>
                mainlog · {visible.length} {visible.length === 1 ? 'entrada' : 'entradas'}
                {filter !== 'all' || search ? ` de ${entries.length}` : ''}
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <button
              ...
              style={{
                ...
                color: autoRefresh ? '#0369A1' : '#64748B',
                transition: 'all 0.15s',
              }}
            >
              <RefreshCw
            ...
            >
              {LIMIT_OPTIONS.map(l => (
                <option key={l} value={l}>{l} linhas</option>
              ))}
            </select>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            >
              ...
              <span style={{
                ...
                background: filter === 'all' ? '#0EA5E9' : '#E2E8F0',
            ...
            {counts.other > 0 && (
              <TypeChip
                type="other"
                active={filter === 'other'}
                onClick={() => setFilter(f => f === 'other' ? 'all' : 'other')}
              />
            )}
          </div>
        ...
        <div ref={bodyRef} style={{ flex: 1, overflowY: 'auto' }}>
          {loading ? (
            <div style={{
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              height: 160, gap: 10, color: '#94A3B8', fontSize: 13,
            }}>
        ...
        <div style={{
          ...
          <span>Pressione <kbd style={{ fontFamily: 'monospace', background: '#F1F5F9', padding: '1px 5px', borderRadius: 4, border: '1px solid #E2E8F0' }}>ESC</kbd> para fechar</span>
