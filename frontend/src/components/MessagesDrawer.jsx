...
const CONFIG = {
  queue: {
    ...
    icon: Inbox,
    endpoint: 'queue',
    limitOptions: null,   // queue não tem limit — usa tudo
    columns: [
...
function CellValue({ col, row }) {
  ...
  if (col.key === 'recipients' && Array.isArray(val)) {
    return (
      <span title={val.join(', ')} style={{ color: 'var(--text)' }}>
        {val[0] || '—'}
        {val.length > 1 && (
  ...
  return (
    <span title={str} style={{ color: str === '—' ? 'var(--dim)' : 'var(--text)' }}>
      {str || '—'}
    </span>
  )
}
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{
          ...
          {/* Limit selector (só para log) */}
          {cfg.endpoint === 'log' && (
            <select
              value={limit}
              onChange={e => setLimit(Number(e.target.value))}
              style={{
                fontSize: 11, borderRadius: 7, border: '1px solid #E2E8F0',
                padding: '5px 8px', color: '#64748B', background: '#F8FAFC',
        ...
        <div style={{ padding: '10px 20px', borderBottom: '1px solid #F1F5F9', flexShrink: 0 }}>
          <input
            type="text"
            placeholder="Filtrar por endereço, host, motivo…"
            value={filter}
            onChange={e => setFilter(e.target.value)}
            style={{
              width: '100%', borderRadius: 8,
              border: '1px solid #E2E8F0', background: '#F8FAFC',
              padding: '7px 12px', fontSize: 12, color: '#0F172A', outline: 'none',
        ...
        <div style={{ flex: 1, overflow: 'auto' }}>
          ...
          {!loading && !error && filtered.length === 0 && (
            <div style={{
              display: 'flex', flexDirection: 'column', alignItems: 'center',
              justifyContent: 'center', height: '100%', gap: 8, color: '#94A3B8',
            }}>
                  ...
                  >
                    {cfg.columns.map(col => (
                      <td
                        ...
                        style={{
                          padding: '9px 12px',
                          maxWidth: col.flex ? 200 : col.width,
